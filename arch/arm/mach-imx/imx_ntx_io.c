...
void ntx_wifi_power_ctrl (int isWifiEnable)
{
	...
	if(np)
	{
		...
		if(pdev) {
//			printk("pdev name= %s\n",pdev->name);
			wifi_power = gpiod_get(&pdev->dev, "wifipwr", GPIOD_ASIS);
			if (IS_ERR(wifi_power)) {
				printk("Get gpio descripter failed\n");
			} else {
				gpiod_direction_output(wifi_power,isWifiEnable);
				gpiod_put(wifi_power);

				wifi_rst = gpiod_get(&pdev->dev, "wifirst", GPIOD_ASIS);
				if (IS_ERR(wifi_rst)) {
					printk("Get wifirst gpio descripter failed\n");
				} else {
					if (isWifiEnable) {
						gpiod_direction_output(wifi_rst, 0);
						msleep (20);
						gpiod_direction_output(wifi_rst, 1);
					}
					else {
						//gpiod_direction_input(wifi_rst);
						gpiod_direction_output(wifi_rst,0);
					}
					gpiod_put(wifi_rst);
				}
				wifi_int = gpiod_get(&pdev->dev, "wifiint", GPIOD_ASIS);
				if (IS_ERR(wifi_int)) {
					printk("Get wifiint gpio descripter failed\n");
				} else {
