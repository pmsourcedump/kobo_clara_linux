...
{
	int iChk;
	printk(KERN_ERR "%s() reset gpio %d\n",__FUNCTION__, zForce_ir_touch_data.rst_gpio);
	gpio_direction_output(zForce_ir_touch_data.rst_gpio, 0);
	msleep(20);
	gpio_direction_input(zForce_ir_touch_data.rst_gpio);
	msleep(500);
...
static int __zForce_ir_touch_init(struct i2c_client *client)
{
	...
	while ((100>count++) && (0xFF != g_zforce_initial_step))
		msleep (10);
...
static int zForce_ir_touch_recv_data(struct i2c_client *client, uint8_t *buf)
{
	...
	rc = i2c_master_recv(client, buf_recv, 2);
	if ( rc < 0 ){
		printk (KERN_ERR "[%s-%d] Error , i2c_master_recv return %d  !!\n",__func__,__LINE__, rc);
...
//			printk ("[%02X %02X %02X %02X %02X %02X %02X]\n",buf[2],buf[3],buf[4],buf[5],buf[6],buf[7],buf[8]);
		...
		case 7:
			if (5 == buf_recv[1])
				printk (KERN_ERR "[%s-%d] command BootComplete (%02X %02X %02X %02X)...\n",__func__,__LINE__,buf[1],buf[2],buf[3],buf[4]);
			else
				printk (KERN_ERR "[%s-%d] command BootComplete (%02X)...\n",__func__,__LINE__,buf[1]);
...
static struct zforce_ts_platdata *zforce_parse_dt(struct device *dev)
{
	...
	zForce_ir_touch_data.intr_gpio = of_get_named_gpio(np, "gpio_intr", 0);
	if (!gpio_is_valid(zForce_ir_touch_data.intr_gpio))
		return -ENODEV;

	err = devm_gpio_request_one(dev, zForce_ir_touch_data.intr_gpio,
				GPIOF_IN, "gpio_zForce_ir_intr");
	if (err < 0) {
		dev_err(dev,"request gpio failed: %d\n", err);
		return err;
	}
	zForce_ir_touch_data.client->irq = gpio_to_irq(zForce_ir_touch_data.intr_gpio);

	zForce_ir_touch_data.rst_gpio = of_get_named_gpio(np, "gpio_rst", 0);
	if (!gpio_is_valid(zForce_ir_touch_data.rst_gpio))
		return -ENODEV;

	err = devm_gpio_request_one(dev, zForce_ir_touch_data.rst_gpio,
				GPIOF_OUT_INIT_HIGH, "gpio_zForce_ir_rst");
	if (err < 0) {
		dev_err(dev,"request gpio failed: %d\n", err);
		return err;
	}
	gpio_set_value(zForce_ir_touch_data.rst_gpio, 0);
	msleep(10);
	gpio_set_value(zForce_ir_touch_data.rst_gpio, 1);
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (0 > zforce_parse_dt(&client->dev)) {
		printk("Error parsing dtb file\n");
		return -ENOMEM;
	}
...
static const struct of_device_id zforce_dt_ids[] = {
	{
		.compatible = "neonode,zforce-ir-touch",
	}, {
		/* sentinel */
	}
};
MODULE_DEVICE_TABLE(of, zforce_dt_ids);

static const struct dev_pm_ops zforce_dev_pm_ops = {
...
static struct i2c_driver zForce_ir_touch_driver = {
	...
	.driver		= {
		...
		.owner = THIS_MODULE,
		.of_match_table = zforce_dt_ids,
#ifdef CONFIG_PM
		.pm = &zforce_dev_pm_ops,
#endif
	},
};
