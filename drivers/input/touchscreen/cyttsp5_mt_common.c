		...
		struct cyttsp5_touch *tch, int num_cur_tch)
{
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		if (tch->abs[CY_TCH_E] == CY_EV_LIFTOFF) {
			...
			g_touch_status &= ~(1<<(t - md->t_min));
			if(0==gptHWCFG->m_val.bUIStyle) {
				/* single touch event for ebrmain*/
				/* Process touch */
				cyttsp5_mt_process_touch(md, tch);

				input_report_abs(md->input, ABS_X ,tch->abs[CY_TCH_X]);
				input_report_abs(md->input, ABS_Y ,tch->abs[CY_TCH_Y]);
				input_report_abs(md->input, ABS_PRESSURE ,0);
				input_report_abs(md->input, BTN_TOUCH, 0);
			}
		...
		if(0==gptHWCFG->m_val.bUIStyle) {
			/* single touch event for ebrmain*/
			input_report_abs(md->input, ABS_X ,tch->abs[0]);
			input_report_abs(md->input, ABS_Y ,tch->abs[1]);
			input_report_abs(md->input, ABS_PRESSURE, 1024);
			input_report_abs(md->input, BTN_TOUCH, 1);
		}
...
static int cyttsp5_setup_input_device(struct device *dev)
{
	...
	if(0==gptHWCFG->m_val.bUIStyle) {
		/* single touch event for ebrmain*/
		__set_bit(ABS_X, md->input->absbit);
		__set_bit(ABS_Y, md->input->absbit);
		__set_bit(ABS_PRESSURE, md->input->absbit);
	...
	if(0==gptHWCFG->m_val.bUIStyle) {
		/* single touch event for ebrmain*/
		input_set_abs_params(md->input, ABS_X, 0, 1439, 0, 0);
		input_set_abs_params(md->input, ABS_Y, 0, 1079, 0, 0);
		input_set_abs_params(md->input, ABS_PRESSURE, 0, 2048, 0, 0);
	}
