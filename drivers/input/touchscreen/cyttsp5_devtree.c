...
static void *create_and_get_mt_pdata(struct device_node *dev_node)
{
	...
	if (!rc) {
		pdata->flags = value;
		printk("%s():cyttsp5 touch flags set by dts=0x%x\n",__FUNCTION__,pdata->flags);
	}
		...
		if(gptHWCFG) {
			...
			if(0==gptHWCFG->m_val.bUIStyle) {
				...
				{	
					pdata->flags |= CY_MT_FLAG_INV_X;
				}
			}
			else if(1==gptHWCFG->m_val.bUIStyle) {
				// Customer UI .
				if(gptHWCFG->m_val.bPCB== 74 || gptHWCFG->m_val.bPCB== 73 ) // E60K00:74 , E60U10:73
				{	
					//pdata->flags |= CY_MT_FLAG_INV_Y ;
				...
				{
					pdata->flags |= CY_MT_FLAG_INV_X|CY_MT_FLAG_INV_Y;
				}
			}
			printk("set cyttsp5 touch flags by hwconfig=0x%x\n",pdata->flags);
		}
	}
#endif //]
		...
		struct device_node *core_node)
{
	...
	if (!rc) {
		pdata->flags = value;
		printk("set cyttsp5 touch flags by dts=0x%x\n",(unsigned)value);
	}
