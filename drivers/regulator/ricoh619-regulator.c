...
static int ricoh61x_regulator_suspend(struct device *dev)
{
	...
	for (reg_id=RICOH619_ID_DC1; reg_id<=RICOH619_ID_LDORTC2; reg_id++) {
		...
		if (gSleep_Mode_Suspend ) {
			...
			switch (reg_id) {
			case RICOH619_ID_DC2:	// Core3_3V3
				ricoh61x_write(to_ricoh61x_dev(rdev), 0x3C, 0xC0);	// set 3V in sleep mode
				ricoh61x_write(to_ricoh61x_dev(rdev), 0x2E, 0x83);	// set PSM in sleep mode
				break;
			case RICOH619_ID_DC3:	// Core2_1V3_ARM
				if(63 != gptHWCFG->m_val.bPCB) {	// T60Q02
			...
			case RICOH619_ID_LDO10:
				if (50==gptHWCFG->m_val.bPCB || 47==gptHWCFG->m_val.bPCB || 54==gptHWCFG->m_val.bPCB || 58==gptHWCFG->m_val.bPCB || 61==gptHWCFG->m_val.bPCB || 63==gptHWCFG->m_val.bPCB || 66==gptHWCFG->m_val.bPCB || 68==gptHWCFG->m_val.bPCB) {
					// E60QFX/ED0Q0X/ED0Q1X/E60QJX/E60QKX/T60Q02/E60U02/E60QP2
					ricoh61x_write(to_ricoh61x_dev(rdev), offset, temp);
				}
				break;
			...
			switch (reg_id) {
			case RICOH619_ID_DC2:	// Core3_3V3
				ricoh61x_write(to_ricoh61x_dev(rdev), 0x3C, 0xD8);	// set 3.3V in sleep mode
