...
static ssize_t vcom_read(struct device *dev, struct device_attribute *attr,char *buf)
{
				...
				13==gptHWCFG->m_val.bDisplayCtrl) 
	{
		...
		iChk = fp9928_get_vcom(fp9928,&iVCOM_mV);
		if(iChk>=0) {
			sprintf (buf,"%dmV\n",iVCOM_mV);
		}
#endif //]FP9928_ENABLED
				...
				16==gptHWCFG->m_val.bDisplayCtrl) 
	{
		...
		iChk = tps6518x_get_vcom(tps6518x,&iVCOM_mV);
		if(iChk>=0) {
			sprintf (buf,"%dmV\n",iVCOM_mV);
		}
#endif //]TPS65185_ENABLED
...
static int k_set_temperature(struct fb_info *info)
{
	...
	if(0==gdwLastUpdateJiffies||time_after(jiffies,gdwLastUpdateJiffies)) {
				...
				13==gptHWCFG->m_val.bDisplayCtrl) 
		{
			...
			iChk = fp9928_get_temperature(fp9928,&giLastTemprature);
#endif //]FP9928_ENABLED
		}
		else
...
static int k_set_vcom(int iVCOM_set_mV)
{
...
#ifdef FP9928_ENABLED//[			
		struct fp9928 *fp9928 = _get_fp9928();
		iRet = fp9928_set_vcom(fp9928,iVCOM_set_mV,0);
		if(iRet>=0) {
			int iVCOM_get_mV;
			if(fp9928_get_vcom(fp9928,&iVCOM_get_mV)>=0) {
				...
				16==gptHWCFG->m_val.bDisplayCtrl) 
	{
		...
		struct tps6518x *tps6518x = _get_tps6518x();
		// MX50+tps16585/MX6SL+tps65185/MX7D+tps65185 .
		iRet = tps6518x_set_vcom(tps6518x,iVCOM_set_mV,0);
		if(iRet>=0) {
			int iVCOM_get_mV;
			if(tps6518x_get_vcom(tps6518x,&iVCOM_get_mV)>=0) {
				vcom_nominal=iVCOM_get_mV*1000;/* save the vcom_nominal value in uV */
			}
		}
#endif //]TPS65185_ENABLED
...
static int k_set_vcom_to_flash(int iVCOM_set_mV)
{
...
#ifdef FP9928_ENABLED//[
		struct fp9928 *fp9928 = _get_fp9928();
		iRet = fp9928_set_vcom(fp9928,iVCOM_set_mV,1);
		if(iRet>=0) {
			int iVCOM_get_mV;
			if(fp9928_get_vcom(fp9928,&iVCOM_get_mV)>=0) {
				vcom_nominal=iVCOM_get_mV*1000;/* save the vcom_nominal value in uV */
				...
				16==gptHWCFG->m_val.bDisplayCtrl)) 
	{
		...
		struct tps6518x *tps6518x=_get_tps6518x();
		// MX50+tps16585/MX6SL+tps65185/MX7D+tps65185 .
		iRet = tps6518x_set_vcom(tps6518x,iVCOM_set_mV,1);
		if(iRet>=0) {
			int iVCOM_get_mV;
			if(tps6518x_get_vcom(tps6518x,&iVCOM_get_mV)>=0) {
				vcom_nominal=iVCOM_get_mV*1000;/* save the vcom_nominal value in uV */
...
static int k_get_vcom(int *O_piVCOM_get_mV)
{
				...
				13==gptHWCFG->m_val.bDisplayCtrl) 
	{
		...
		iRet = fp9928_get_vcom(fp9928,O_piVCOM_get_mV);
#endif //]FP9928_ENABLED
		}
	}
	else
		...
		if(7==gptHWCFG->m_val.bDisplayCtrl) {
			...
			
			{
						...
						13==gptHWCFG->m_val.bDisplayCtrl) 
					{
#ifdef FP9928_ENABLED//[			
						if(0!=glVCOM_uV) {
							if(fp9928_set_vcom(fp9928,(int)(glVCOM_uV/1000),0)<0) {
								WARNING_MSG("%s(),FP9928 vcom set fail !\n",__FUNCTION__);
							}
						}
						fp9928_get_vcom(fp9928,&vcom_mV);
#else //][!FP9928_ENABLED
						iChk=0;
						vcom_mV=-2500;
#endif //]FP9928_ENABLED
...
#ifdef TPS65185_ENABLED//[
						if(0!=glVCOM_uV) {
							if(tps6518x_set_vcom(tps6518x,(int)(glVCOM_uV/1000),0)<0) {
						...
						iChk = tps6518x_get_vcom(tps6518x,&vcom_mV);
#else //]!TPS65185_ENABLED
						iChk=0;
						vcom_mV=-2500;
#endif //]TPS65185_ENABLED
					}
