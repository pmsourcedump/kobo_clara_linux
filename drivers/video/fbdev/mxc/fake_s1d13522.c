...
static int read_file(char *filename, char *pBuffer)
{
  ...
  if (fd<0) {
  	printk (KERN_ERR"[%s-%d] failed open \"%s\",(fd=%d)\n",
			__func__,__LINE__,filename,fd);
  }
	else {
		printk(KERN_DEBUG);

  	tSz = sys_read(fd, pBuffer, 4096);
  ...
  fd = sys_open(filename, O_WRONLY|O_CREAT, 0644);
	...
	if (fd<0) {
  	printk(KERN_ERR"[%s-%d] failed open \"%s\",(fd=%d)\n",
		...
		fsz_chk = sys_lseek(fd,fsz_temp,SEEK_SET);
		if(fsz_temp!=fsz_chk) {
			printk(KERN_ERR"[WARNING] %s() : seek size check fail %d!=%d\n",
				__FUNCTION__,fsz_chk,fsz_temp);
		}
		else {
			tSz = sys_write(fd, (char *)data, dwDataSize);
			file = fget(fd);
			if(file) {
				vfs_write(file,(char *)data,dwDataSize,&pos);
				fput(file);
			}
			if(dwDataSize!=tSz) {
				printk (KERN_ERR"[%s-%d] failed write %s,%d!=%d\n",
					__func__,__LINE__,filename,(int)tSz,(int)dwDataSize);
			}
			iRet = (int)(tSz);
		}
		sys_close(fd);
