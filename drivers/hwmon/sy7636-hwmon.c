...
int sy7636_get_temperature(struct sy7636 *sy7636,int *O_piTemperature)
{
	...
	sy7636_reg_read(sy7636,REG_SY7636_THERM, &reg_val);
...
int sy7636_set_vcom(struct sy7636 *sy7636,int iVCOMmV,int iIsWriteToFlash)
{
	...
	do
	{
		...
		if(sy7636_reg_read(sy7636,REG_SY7636_VCOM_ADJ2,&regVCOM2)<0) {
			printk(KERN_ERR"%s():SY7636 VCOM2 reading error !\n",__FUNCTION__);
			iRet = -4;
			break;
		}

		regVCOM1 = (unsigned char)(wVCOM_val&0xff);
		if(sy7636_reg_write(sy7636,REG_SY7636_VCOM_ADJ1, regVCOM1)<0) {
		...
		if(sy7636_reg_write(sy7636,REG_SY7636_VCOM_ADJ2, regVCOM2)<0) {
...
int sy7636_get_vcom(struct sy7636 *sy7636,int *O_piVCOMmV)
{
	...
	if(!sy7636->vcom_setup) {
		int iVCOMmV = sy7636->vcom_uV/1000;
		if(O_piVCOMmV) {
			*O_piVCOMmV = iVCOMmV;
	 ...
	 */
	sy7636_reg_read(sy7636,REG_SY7636_VCOM_ADJ1, &vcom_reg_val);
	sy7636_reg_read(sy7636,REG_SY7636_VCOM_ADJ2, &reg_val);
	vcom_reg_val |= reg_val<<1;
