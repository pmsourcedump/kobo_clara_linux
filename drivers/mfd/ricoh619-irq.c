...
static const inline struct ricoh61x_irq_data * irq_to_ricoh61x_irq(struct ricoh61x *ricoh61x, int irq)
{
	struct irq_data *data = irq_get_irq_data(irq);
	return &ricoh61x_irqs[data->hwirq];
}
...
{
	struct ricoh61x *ricoh61x = irq_data_get_irq_chip_data(irq_data);
	const struct ricoh61x_irq_data *data= irq_to_ricoh61x_irq(ricoh61x,irq_data->irq);
...
static irqreturn_t ricoh61x_irq(int irq, void *data)
{
	...
	ret = ricoh61x_read(ricoh61x->dev, RICOH61x_INT_IR_SYS, &ricoh619_pwr_key_reg);
	/* printk("PMU: %s: irq=%d\n", __func__, irq); */
...
//				return IRQ_HANDLED;
	...
	for (i = 0; i < RICOH61x_NR_IRQS; ++i) {
		...
		{
			isIrqHandled = 1;
			int cur_irq = irq_find_mapping(ricoh61x->irq_domain, i);
			if (cur_irq)
	...
	if (! isIrqHandled) {
		...
		for (i = 0; i < MAX_INTERRUPT_MASKS; ++i) {
			if (int_sts[i])
			printk(KERN_INFO "PMU2: %s: irq_mon_add %02X, int_sts[%d]=0x%x main_int_type[%d]=0x%X\n",
							 __func__, irq_mon_add[i], i, int_sts[i], i, main_int_type[i]);
		}
	}
...
static struct irq_chip ricoh61x_irq_chip = {
	...
	.irq_enable = ricoh61x_irq_unmask,
	.irq_disable = ricoh61x_irq_mask,
...
static int ricoh61x_irq_domain_map(struct irq_domain *d, unsigned int irq,
					irq_hw_number_t hw)
{
	struct ricoh61x *ricoh61x = d->host_data;

	irq_set_chip_data(irq, ricoh61x);
	irq_set_chip_and_handler(irq, &ricoh61x_irq_chip, handle_edge_irq);
...
static struct irq_domain_ops ricoh61x_irq_domain_ops = {
	.map = ricoh61x_irq_domain_map,
};
				...
				struct ricoh619_platform_data *pdata)
{
	...
	if (!irq) {
		dev_warn(ricoh61x->dev, "No interrupt support, no core IRQ\n");
		return -EINVAL;
	}
	...
	if (pdata->irq_gpio && !ricoh61x->chip_irq) {
		ricoh61x->chip_irq = gpio_to_irq(pdata->irq_gpio);
	...
	domain = irq_domain_add_linear(NULL, RICOH61x_NR_IRQS,
					&ricoh61x_irq_domain_ops, ricoh61x);
	if (!domain) {
		dev_err(ricoh61x->dev, "could not create irq domain\n");
		return -ENODEV;
	}
	ricoh61x->irq_domain = domain;
	ret = devm_request_threaded_irq(ricoh61x->dev,ricoh61x->chip_irq, NULL, ricoh61x_irq, flags |IRQF_ONESHOT, "ricoh619", ricoh61x);
//	ret = devm_request_threaded_irq(ricoh61x->dev,ricoh61x->chip_irq, NULL, ricoh61x_irq, IRQF_TRIGGER_FALLING | IRQF_ONESHOT , "ricoh619", ricoh61x);

	irq_set_irq_type(ricoh61x->chip_irq, irq_type);
	enable_irq_wake(ricoh61x->chip_irq);
